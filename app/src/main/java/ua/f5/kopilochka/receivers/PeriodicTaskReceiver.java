...
import ua.f5.kopilochka.http.Connect;
    ...
    public void http_result(int type, String result) {
        switch (type){
            case Const.getNotices:
                ...
                if(notices != null) {
                    if (Methods.PutNotificationInBase(context, notices)) {
            ...
            case Const.getActions:
                ...
                if(actions != null) {
                    if (Methods.PutActionInBase(context, actions)) {
                        //Create notification for new Action
                        CreateNotification("Повідомлення", "Отримано нові акції", "");
                    }
                }
                ArrayList<ProductGroup> productList = Parser.getProductGroupArray(context, result);
                if(productList != null) {
                    if (Methods.PutGroupsInBase(context, productList)) {
